...
mark.hit { background: #FFE58A; color: inherit; padding: 0 1px; border-radius: 2px; }
mark.hit.current { background: #F5A623; box-shadow: 0 0 0 2px #C38A24; }
.hitnav { display: inline-flex; align-items: center; gap: 6px; font-size: 0.88em; color: var(--muted); white-space: nowrap; }
.hitnav[hidden] { display: none; }
.searchbar .hitnav button { padding: 4px 10px; }
@media print { mark.hit, mark.hit.current { background: none; box-shadow: none; } }
</style>"""
...
(function() {
  ...
  // Маркирането се пропуска при над MAX_MARKED съвпадащи раздела или думи под 3
  // знака: иначе първата буква в полето би маркирала десетки хиляди места.
  var MAX_MARKED = 500, marked = [], marks = [], cur = -1, highlight = false;
  var nav = document.createElement('span');
  nav.className = 'hitnav';
  nav.hidden = true;
  nav.innerHTML = '<button type="button" id="hitPrev" title="Previous match (Shift+Enter)">&#8593;</button>'
    + '<span id="hitPos"></span>'
    + '<button type="button" id="hitNext" title="Next match (Enter)">&#8595;</button>';
  document.getElementById('resultCount').insertAdjacentElement('afterend', nav);
  var pos = document.getElementById('hitPos');
  ...
  function filter(ch) {
    ...
    ch.live.forEach(function(n) { if (n.nodeType === 1) clearMarks(n); });
    if (highlight) ch.units.forEach(function(u, i) { if (u.match) markIn(units[i]); });
  }
  function clearMarks(root) {
    Array.prototype.forEach.call(root.querySelectorAll('mark.hit'), function(m) {
  ...
  function markIn(el) {
    ...
    nodes.forEach(function(node) {
      var text = node.nodeValue, low = text.toLowerCase(), ranges = [], merged = [];
      marked.forEach(function(t) {
        for (var i = low.indexOf(t); i !== -1; i = low.indexOf(t, i + t.length)) ranges.push([i, i + t.length]);
      });
  ...
  function refreshNav() {
    marks = Array.prototype.slice.call(document.querySelectorAll('mark.hit'));
    cur = -1;
    nav.hidden = !terms.length;
    if (!terms.length) pos.textContent = '';
    else if (!highlight) pos.textContent = marked.length ? 'too many to highlight' : 'type 3+ letters to highlight';
    else pos.textContent = marks.length ? ('0 / ' + marks.length) : '';
  }
  function go(step) {
    if (!marks.length) return;
    if (cur >= 0) marks[cur].classList.remove('current');
    cur = (cur + step + marks.length) % marks.length;
    marks[cur].classList.add('current');
    marks[cur].scrollIntoView({ block: 'center' });
    pos.textContent = (cur + 1) + ' / ' + marks.length;
  }
  ...
  function search() {
    ...
    marked = terms.filter(function(t) { return t.length >= 3; });
    highlight = marked.length > 0 && shown <= MAX_MARKED;
    chapters.forEach(function(ch) {
      var wasLive = !!ch.live;
      if (!terms.length) { ch.shell.classList.remove('hidden'); if (ch.bySearch) collapse(ch); }
      else if (ch.hits) expand(ch, true);
  ...
  document.getElementById('hitNext').addEventListener('click', function() { go(1); });
  document.getElementById('hitPrev').addEventListener('click', function() { go(-1); });
  ...
  search();
  follow();
  // дошъл от връзка с дума: веднага към първото маркирано място
  if (q && !location.hash) go(1);
})();
</script>"""
